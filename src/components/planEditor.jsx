...
export function usePlanItemsEditor(initialItems) {
  ...
  // 시판 제품의 "팩"/"중량" 입력 방식 전환 - 재료의 큐브/그램 토글과 동일한 방식.
  // qty(팩 수, 재고 차감 단위)는 그대로 두고 표시용 제공량(gramsOverride)만 켜고 끔
  const setProductMode = (key, mode) => setItems((p) => p.map((it) => {
    if (keyOf(it) !== key) return it;
    if (mode === "gram") return { ...it, gramsOverride: it.gramsOverride != null ? it.gramsOverride : it.qty * (it.packG || 0) };
  ...
  return { items, setItems, upQty, upUnit, upGrams, setMode, setProductMode, rm, addNames, addProduct, replaceFrom };
}

export function PlanItemsEditor({ editor }) {
  const { state } = useStore();
  const { items, upQty, upUnit, upGrams, setMode, setProductMode, rm } = editor;
  return (
    <div>
