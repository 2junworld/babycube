...
function seedState() {
  ...
  return {
    ...
    settings: { timeFmt: "24h", frozenAlertDays: 3, fridgeAlertDays: 1, fridgeKeepDays: 2 },
...
function WeekTable({ startISO, onPickDay }) {
  ...
  const cols = `30px repeat(${labels.length}, minmax(48px, 1fr))`;
  const t = todayISO();
  return (
    <div style={{ overflowX: wide ? "auto" : "visible" }}>
      <div style={{ border: `1px solid ${C.border}`, borderRadius: 12, overflow: "hidden", minWidth: wide ? 30 + labels.length * 58 : "auto" }}>
        <div style={{ display: "grid", gridTemplateColumns: cols, background: C.sageLight, padding: "7px 6px" }}>
          <span style={{ fontSize: 9.5, fontWeight: 700, color: C.sageDeep }}>요일</span>
          {labels.map((h) => <span key={h} style={{ fontSize: 9.5, fontWeight: 700, color: C.sageDeep, textAlign: "center" }}>{h}</span>)}
        </div>
        {days.map((iso, i) => {
          ...
          return (
            <button key={iso} onClick={() => onPickDay(iso)} style={{ display: "grid", gridTemplateColumns: cols, padding: "9px 6px", width: "100%", textAlign: "left",
              borderTop: i === 0 ? "none" : `1px solid ${C.border}`, background: isToday ? C.sageLight : C.surface, border: "none", cursor: "pointer" }}>
              <div><div style={{ fontSize: 11.5, fontWeight: 800, color: isToday ? C.sageDeep : C.ink }}>{WD[dow]}</div><div style={{ fontSize: 8.5, color: C.muted }}>{iso.slice(5)}</div></div>
              {labels.map((lab) => {
                const its = find(lab);
                return (
                  <div key={lab} style={{ padding: "0 3px" }}>
                    <MealItemList items={its} fontSize={9.5} />
                    {its && its.length > 0 && <div style={{ fontSize: 8.5, color: C.muted, fontWeight: 700, marginTop: 2 }}>{totalG(state, its)}g</div>}
                  </div>
                );
              })}
            </button>
