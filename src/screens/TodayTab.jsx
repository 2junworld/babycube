...
/* =====================================================================
   하루치 끼니 카드 목록 - 계획(state.plans[date])과 실제 기록(state.logs[date])을 매칭해
   끼니별로 "예정/대기/완료" 상태와 기록하기 버튼을 보여줌. 오늘 탭이 원래 쓰던 로직을
   date를 매개변수로 받도록 분리해, 급여표(월별)에서 지난 날짜를 선택했을 때도 오늘 탭과
   동일한 화면(계획 확인 후 기록하기)으로 진입할 수 있게 함(개선 제안 반영)
   ===================================================================== */
export function DayFeedingCards({ date, go }) {
  ...
  const meals = plan.map((m) => {
    ...
    // 오늘: 계획 시간이 지났는데 아직 안 먹였으면 "대기". 지난 날짜: 하루가 이미 다 지났으므로
    // 시간 비교 없이 기록 안 된 계획은 전부 "대기"로 봄
    let status = "예정";
    if (log) status = "완료";
    else if (isToday ? m.time < nowHM : date < t) status = "대기";
    return { ...m, log, status };
  });
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      {cards.length === 0 && (
        <div style={{ textAlign: "center", padding: "30px 0", fontSize: 12.5, color: C.muted }}>
          {isToday ? "오늘" : `${date.slice(5)}에`} 계획된 끼니가 없습니다.<br />식단표 탭에서 계획하거나, 아래 '바로 기록'으로 먹인 끼니를 기록해 보세요.
        </div>
      )}
      {cards.map((m) => {
        ...
        return (
          <div key={m.id} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: 14 }}>
            ...
            <div className="flex items-center justify-between" style={{ marginTop: 10, paddingTop: 10, borderTop: `1px dashed ${C.border}` }}>
              ...
              <button onClick={() => go("feed", { date, planMeal: m.adhocOnly ? null : m, existingLog: m.log })}
      ...
      <button onClick={() => go("feed", { date })} className="flex items-center justify-center"
...
/* =====================================================================
   지난 날짜 급여 기록 화면 - 급여표(월별)에서 날짜를 선택해 진입(개선 제안 반영).
   오늘 탭과 동일하게 그 날짜의 계획을 먼저 보여주고 끼니별로 기록하게 해서,
   계획 없이 바로 빈 기록 화면이 뜨던 것과 달리 오늘 탭과 일관된 경험을 제공함
   ===================================================================== */
export function DayRecordScreen({ date, onBack, go }) {
  return (
    <div style={{ paddingBottom: 90 }}>
      <SubHeader title={`${date.slice(5)} 급여 기록`} onBack={onBack} />
      <div style={{ padding: "6px 18px 0" }}>
        <DayFeedingCards date={date} go={go} />
      </div>
    </div>
  );
}
